...
static void insertNode(TreeNode* t) {
    ...
    case StmtK:
        switch (t->kind.stmt) {
        case AssignK: case ReadK:
            /* Not yet in table, so treat as new definition. */
            if (st_lookup(t->attr.name) == -1) st_insert(t->attr.name, t->lineno, location++);
            /* Already in table, so ignore location, add line number of use only. */
            else st_insert(t->attr.name, t->lineno, 0);
            break;
        case IntK: case FloatK: {
            ...
            for (i = 0; i < MAXCHILDREN; i++) {
                ...
                if (child->nodekind == StmtK && child->kind.stmt == AssignK) {
                    name = child->attr.name;
                }
                else if (child->nodekind == ExpK && child->kind.exp == IdK) {
                    name = child->attr.name;
                }
                if (name != NULL) {
                    if (st_lookup(name) == -1) {
...
static void checkNode(TreeNode* t) {
    ...
    case ExpK: switch (t->kind.exp) {
        ...
        break;
    case StmtK: switch (t->kind.stmt) {
        case IfK:
            if (t->child[0]->type != Boolean) typeError(t->child[0], "if test is not Boolean");
            break;
        case AssignK: {
            ExpType leftType = st_lookup_type(t->attr.name);
            if (
                (leftType == Integer && t->child[0]->type != Integer)
