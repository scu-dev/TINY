...
void writeInstruction(int loc) {
    ...
    if ((loc >= 0) && (loc < IADDR_SIZE)) {
        ...
        case opclRM:
        case opclRA:
            if (iMem[loc].iop == opLDC) printf("%.10g(%1d)", iMem[loc].farg2, iMem[loc].iarg3);
            else printf("%3d(%1d)", iMem[loc].iarg2, iMem[loc].iarg3);
            break;
        }
        printf("\n");
    }
...
void printMachineValue(double value) {
    ...
    if (value == (double)ivalue) printf("%d", ivalue);
    else printf("%.10g", value);
}
    ...
    while (!feof(pgm)) {
        ...
        if (nonBlank()) {
            ...
            case opclRA:
                ...
                if (!skipCh(',')) return error("Missing comma", lineNo, loc);
                if (op == opLDC) {
                    if (!getFloatNum()) return error("Bad displacement", lineNo, loc);
                    arg2 = (int)fnum;
                }
                else {
...
STEPRESULT stepTM() {
    ...
    case opOUTS: {
        ...
        if ((stringIndex >= 0) && (stringIndex < STRING_TABLE_SIZE) && stringTableSet[stringIndex]) {
...
int doCommand() {
    ...
    case 'i':
        ...
        if (!atEOL()) printf("Instruction locations?\n");
        else {
    ...
    case 'd':
        ...
        if (!atEOL()) printf("Data locations?\n");
        else {
