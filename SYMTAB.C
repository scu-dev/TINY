...
/* Procedure `st_insert_typed` inserts line numbers and memory locations into the symbol table. */
...
/* Function `st_lookup_type` returns the declared type of a variable. */
    ...
    for (i = 0; i < SIZE; ++i) {
        if (hashTable[i] != NULL) {
            ...
            while (l != NULL) {
                ...
                fprintf(listing, "%-8s  ", (l->type == Float) ? "float" : "int");
                while (t != NULL) {
